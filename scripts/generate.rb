...
metadata.components.each do |component|
  ...
  # Release not templates
  template_path = "#{TEMPLATES_DIR}/docs/resources/release-notes/by-component/#{component.type.pluralize}/#{component.name}.md.erb"

  if !File.exists?(template_path)
    contents = templates.component_release_notes_default(component)
    File.open(template_path, 'w+') { |file| file.write(contents) }
  end
end

#
# Create missing release templates
#

metadata.releases.to_h.each do |version_string, release|
  template_path = "#{TEMPLATES_DIR}/docs/resources/release-notes/by-version/#{version_string}.md.erb"

  if !File.exists?(template_path)
    contents = templates.release_notes_default(release)
    File.open(template_path, 'w+') { |file| file.write(contents) }
  end

  template_path = "#{TEMPLATES_DIR}/docs/assets/v#{version_string}.svg.erb"

  if !File.exists?(template_path)
    contents = templates.release_svg_default(release)
    File.open(template_path, 'w+') { |file| file.write(contents) }
  end
end
